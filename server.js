...
dispatcher.onPost("/recalc", function(req, res) {
    res.writeHead(200, {'Content-Type': 'text/plain'});
	var p = JSON.parse(req.body);
	var resultC = recalculate(p.arc, p.td_arc, p.curr_arc, p.bulk_ar, p.curr_bulk_ar, p.gef, p.tdn, p.let_p1, p.let_p2, p.loantype);
	res.end(JSON.stringify(resultC, null, 2));
});
...
var cancelledl;
   ...
   var cl = rl;
	...
	return ["£" + rlo + "!", loantype + "!", loancomp + "!", cancelledy + " years, " + cancelledm + " months" + "!", "£" + eso + "!", "£" + arto + "!", "£" + artmo + "!", shifo + "%" + "!", gef + "!", tdn +"!", let_p1 + "!", let_p2];
}

/************************************************************************************/
/********************************* RECALCULATE FUNCTION *****************************/
/************************************************************************************/
function recalculate(arc, td_arc, curr_arc, bulk_ar, curr_bulk_ar, gef, tdn, let_p1, let_p2, loantype) {
    ...
    return [loancomp2 + "!", "£" + eso2 + "!", "£" + arto2 + "!", "£" + artmo2 + "!", shifo2 + "%"];
